kaCacheBust = 'bust=' + (new Date()).getTime();
define([
    './kjs/Kare.utils'
], (utils) => {
        ...
        constructor: () => {
            ka = $.extend(self, {
                userID: (function() {
                    ...
                    let storageUserID = localStorage.getItem('kareuserid');
                    if (storageUserID) {
                        userID = storageUserID;
                        return userID;
                    }
                    userID = 'KURxxxxxxxxxxxx4xxxyxxxxxxxxxxxxxxx'.replace(/[xy]/g, function(c) {
                        var r = Math.random() * 16 | 0,
                            v = c == 'x' ? r : (r & 0x3 | 0x8);
                        return v.toString(16);
                    });
                    localStorage.setItem('kareuserid', userID);
                    return userID;
                })(),
                utils: utils,
                env: {
                    parameters: _parseURLParameters(location)
                },
                glog: (option) => {
                    ...
                    let str = ka.userID + '##' + option.tag + '##' + new Date();
                    console.log('glog', str);
                }
            });
            ka.utils.initialization(ka);
            registerErrorHandler(ka);
        },
        data: {},
        resources: [],
        changeView: (name, option) => {
            loadModule(name, option, 'all');
        },
        loadView: (name, option) => {
            loadModule(name, option, 'part');
        },
        log: function() {
            try {
    ...
    function loadModule(name, option, loadType = 'part') {
        require(['./view/' + name + '.js?bust=' + kaCacheBust, 'text!./view/' + name + '.html?bust=' + kaCacheBust],
            (module, html) => {
                html = html.replace(/Fragment/g, 'div');
                html = html.replace(/{scoped}/g, name);
                html = html.replace(/#scoped/g, '#' + name);
                module.ns = name;
                module.$ = function(inSelector) {
                    return $('#' + name + ' ' + inSelector);
                }
                if (option && option.targetID) {
                    $(option.targetID).html(html);
                } else if (loadType == 'part') {
                    $(html).appendTo('#wrapper-view');
                } else {
                    $('#wrapper-view').html(html);
                }

                module.initialization(self);
                module.mount(option);
    ...
    function _parseURLParameters(location) {
        var result = {};
        ...
        window.onerror = function(message, url, lineNumber, column, errorObj) {
            setTimeout(function() {
                try {
                    ...
                    var exceptionErr = ['checkDomStatus', 'setuserNo is not defined', 'isApplication is not defined', 'cafe is not defined', 'PAPP_MENU_TYPE'];
                    ...
                    ka.ga({
                        eventAction: 'SCRIPTERROR#' + info, //서브화면명 
                        eventLabel: pid //이벤트명
                    });
                } catch (e) {
